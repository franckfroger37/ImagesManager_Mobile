...
export const updateProductDescription = async (productId, newDescription, settings) => {
  const { wooUrl, consumerKey, consumerSecret } = settings;
  const response = await fetch(
    `${wooUrl}/wp-json/wc/v3/products/${productId}?consumer_key=${consumerKey}&consumer_secret=${consumerSecret}`,
    {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ short_description: newDescription, description: newDescription }),
    }
  );
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
